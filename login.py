...
class Login_window:
    def __init__(self, root):
        self.root = root
        self.root.title("Login Window - Student Result Management System")
    ...
    def forget_password_window(self):
        if self.txt_email.get() == "":
            ...
            try:
                ...
                con.close()
                if row is None:
                    messagebox.showerror("Error", "Please enter a registered email address", parent=self.root)
                else:
                    self.root2 = Toplevel()
